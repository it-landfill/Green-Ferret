...
void loop() {
	...
	// Get GPS data
	GpsPoint point = gpsGetLocation();
	GpsPoint lastPoint = gpsGetLastPoint();

	if (point != NULL && lastPoint != NULL) {
		float distanceTravelled = getDistance(settings.distanceMethod, lastPoint, point);

		if((settings.trigger == 0 && (distanceTravelled >= settings.distance || lastPoint->timestamp == 0)) || (settings.trigger == 1 && currentTime >= settings.time*1000*sendCounter)) {
			...
			if(settings.trigger == 0) gpsUpdateGPSPoint();
			else sendCounter++;
